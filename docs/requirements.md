...
本项目旨在开发一款基于 **Android AccessibilityService (无障碍服务)** 的自动化点击助手应用。用户无需Root权限，即可通过应用实现屏幕自动点击、滑动、多点触控等操作。应用的核心特色在于**可视化脚本录制**、**脚本库管理**，以及支持**多实例独立脚本快捷键的悬浮窗交互模式**，以适应例如MOBA游戏快速换装、技能连招等对响应速���和多操作并发性要求高的场景。
...
*   **随机化参数 (Randomization Parameters)**：
    *   **坐标偏移 (Coordinate Offset)**：用户可设置一个半径值 R (例如 10像素)。
        *   *逻辑*：每次执行点击时，实际坐标 = 设定的中心坐标 (x, y) + 随机偏移 (dx, dy)，其中 dx, dy 在 [-R, +R] 范围内。
    *   **时长波动 (Duration/Delay Fluctuation)**：用户可设置一个时间范围 T (例如 100毫秒)。
        *   *逻辑*：每次执行等待或按压时，实际时长 = 设定的基准时长 + 随机时长，范围在 [-T/2, +T/2] 之间。
    *   **参数层级**：
        *   **全局设置**：作为默认值应用到所有脚本。
...
*   **手势捕获**：
    *   应用生成一个**全屏透明覆盖层 (Overlay)**，拦截用户的触摸事件。
    *   记录用户的手指按下、移动、抬起的坐标轨迹和时间戳。
    *   *注：由于系统安全限制，录制时通常无法同时操作底层APP（即录制时��阻断点击），录制完成后再回放。建议采用“透明层阻断录制”的标准方案。*
...
*   **定位**：全局总控，用于应用的设置、脚本管理、录制和退出。
*   **形态**：长条形或展开的功能面板，功能较全，可在主应用界面或录制模式下使用。
*   **核心功能**：
    *   **进入录制/编辑模式**：启动录制流程，并显示必要的编辑控件。
    *   **脚本库显隐管理**：弹出一个列表，列出所有脚本。用户可以勾选哪些脚本需要**“显示为独立脚本快捷键”**在屏幕上。
...
*   **交互逻辑**：
    *   **单击执行**：**立即执行**该按钮对应的脚本逻辑（无延迟，响应优先）。
        *   *并发策略*：若此时已有脚本在运行，默认**打断当前脚本**并立即执行新脚本（适应MOBA瞬息万变的战况）。
    *   **长按调整**：长按按钮可进入“调整模式”，此时可以：
        *   **拖拽移动位置**：调整按钮在屏幕上的精确位置。
...
// 脚本数据模型示例
{
  ...
  "global_random_offset": 10,          // 全局坐标随机偏移 ±10px
  "global_random_delay": 200,          // 全局延迟随机波动 ±200ms
